...
start = 1               # 자를 수 있는 나무의 최소 길이
end = max(heights)      # 자를 수 있는 나무의 최대 길이
result = 0
...
while start <= end:
    ...
    for height in heights:
        if height > point:                      # 계산한 중간값이 나무의 높이보다 높다면
            cut_height += (height - point)      # 자르고 남은 길이를 더한다.

    if cut_height >= M:           # 적어도 M미터의 나무라고 했기 때문에 나무길이를 M미터 이상 가져가는 경우도 고려
        result = point
        start = point + 1
    else:                         # 자른 나무의 합이 M미터보다 작다면
        end = point - 1           # end값 조절
